...
#if defined(LOGIT_WITH_OTLP) && defined(LOGIT_HAS_ZLIB)
...
struct RequestCapture {
    ...
    bool has_content_encoding_gzip = false;
};
...
void start_server(HttpServer& server, std::thread& thread, RequestCapture& capture, unsigned short port) {
    ...
    server.resource["^/v1/logs$"]["POST"] = [&capture](std::shared_ptr<HttpServer::Response> response,
                                                         std::shared_ptr<HttpServer::Request> request) {
        {
            ...
            if (it != request->header.end() && it->second == "gzip") {
                capture.has_content_encoding_gzip = true;
            }
...
int main() {
    const unsigned short port = 43182;
    ...
    config.path = "/v1/logs";
    config.format.service_name = "gzip-test";
    ...
    for (int i = 0; i < 3; ++i) {
        logit::LogRecord record(
            ...
            "test.cpp", 100 + i, "test_func", "gzip test", "",
            -1, false, false, false);
        logger->log(record, "gzip compression test message " + std::to_string(i));
    }
    ...
    assert(capture.has_content_encoding_gzip);
    assert(capture.last_body.size() >= 2);
    assert(static_cast<unsigned char>(capture.last_body[0]) == 0x1f);
    assert(static_cast<unsigned char>(capture.last_body[1]) == 0x8b);
