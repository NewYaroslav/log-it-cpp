...
static bool test_dedicated_executor_returns_true() {
    ...
    cfg.directory = "test_file_queue_cfg";
    cfg.async = true;
...
static bool test_global_executor_returns_false() {
    logit::FileLogger::Config cfg;
    cfg.directory = "test_file_queue_cfg_global";
    cfg.async = true;
...
static bool test_shutdown_returns_false() {
    logit::FileLogger::Config cfg;
    cfg.directory = "test_file_queue_cfg_shut";
    cfg.async = true;
    cfg.use_dedicated_executor = true;
    logit::FileLogger logger(cfg);
    logger.shutdown();
...
int main() {
    ...
    std::cout << (ok ? "PASS" : "FAIL") << ": file_logger_set_queue_config" << std::endl;
    return ok ? 0 : 1;
}
