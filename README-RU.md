...
Библиотека сочетает простоту макросов для логирования, аналогичную **IceCream-Cpp**, и возможности настройки бэкендов и форматов логов, как в **spdlog**.
...
- Основное логирование:
	...
	- `LOG_T(...)`: Логирование сообщения уровня TRACE.
	- `LOG_T0()`: Логирование сообщения уровня TRACE без аргументов.
	- `LOG_T_NOARGS()`: Логирование сообщения уровня TRACE без аргументов.
	
- Логирование с форматированием:

	- `LOG_TF(fmt, ...)`: Логирование форматированного сообщения уровня TRACE с использованием строк формата.
	- `LOG_T_PRINT(...)`: Логирование сообщения уровня TRACE с выводом каждого аргумента.
	- `LOG_T_PRINTF(fmt, ...)`: Логирование форматированного сообщения уровня TRACE с использованием форматирования в стиле printf.
	- `LOG_TP(...)`: Псевдоним для `LOG_T_PRINT(...)`.
	- `LOG_TPF(fmt, ...)`: Псевдоним для `LOG_T_PRINTF(fmt, ...)`.
	
- Альтернативные макросы уровня `TRACE`:

	- `LOG_TRACE(...)`: Логирование сообщения уровня TRACE (то же, что и `LOG_T(...)`).
	- `LOG_TRACE0()`: огирование сообщения уровня TRACE без аргументов (то же, что и `LOG_T0()`).
	- `LOG_TRACE_NOARGS()`: Логирование сообщения уровня TRACE без аргументов (то же, что и `LOG_T_NOARGS()`).
	- `LOG_TRACEF(fmt, ...)`: Логирование форматированного сообщения уровня TRACE (то же, что и `LOG_TF(fmt, ...)`).
	- `LOG_TRACE_PRINT(...)`: Логирование сообщения уровня TRACE с выводом каждого аргумента (то же, что и `LOG_T_PRINT(...)`).
	- `LOG_TRACE_PRINTF(fmt, ...)`: Логирование форматированного сообщения уровня TRACE с использованием printf-стиля форматирования (то же, что и `LOG_T_PRINTF(fmt, ...)`).
