...
`LogRecordView` records and avoid depending on backend-specific storage:
...
int main() {
    ...
    const uint64_t callback_id = LOGIT_ADD_LOG_CALLBACK(
        backend_index,
        ([&live_updates](const logit::LogRecordView& record) {
...
`LogRecordView` after the backend has written the record. This is the preferred
fallback-friendly API between `MemoryLogger` and `MdbxLogger`.
